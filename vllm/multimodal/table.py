...
class TableCol(TypedDict, total=False):
    name: Required[Union[str, int, float]]
    dtype: Required[str]
...
def table_custom_tokenize_encode(prompt: str, tokenizer: PreTrainedTokenizer,
    ...
    placeholder_token = getattr(
        hf_config, "placeholder_token", None) or getattr(
            hf_config.encoder_config, "insert_embs_token", None)
    placeholder_token_id = getattr(
        hf_config, "placeholder_token_id", None) or getattr(
            hf_config.encoder_config, "insert_embs_token_id", None)
    ...
    prompt_chunks = [
        tokenizer(
            ...
            truncation=True,
        ).input_ids for e in prompt.split(placeholder_token)
    ]
    ...
    for x in insert_separator(prompt_chunks, [placeholder_token_id] *
                              _repeated_count * (offset + 1)):
        input_ids.extend(x[offset:])
    return input_ids


def __dataframe_info_simple(table: Union[ColumnsTable, MarkdownTable],
...
def get_full_tables_info(
    ...
    return ("".join(__build_table_question(tables, model_config))
            if return_text else __build_table_question(tables, model_config))


def get_full_tarbular_text_prompt(placeholder_token_str: str,
                                  table_infos: List[str],
                                  text_prompt: str) -> str:
