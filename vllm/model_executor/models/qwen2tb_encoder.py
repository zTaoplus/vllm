...
def get_embedded_table(table:Table):

    df_col_count = len(table["cols"])

    tb = np.array(table['values'])
    if len(tb) > VLLM_TABLE_MAX_ROWS:
        tb = tb[np.random.choice(tb.shape[0], size=50, replace=False)]
    _, num_cols = tb.shape[0], tb.shape[1]
    anchor_row_num = tb.shape[0]
    anchor_table = tb.reshape(-1)
    max_length = 64
    anchor_table = TABGPT_ENCODER.tokenizer(anchor_table.tolist(), padding='max_length', truncation=True, max_length=max_length, return_tensors='pt')                
...
def get_encoder_output(tables:t.List[Table]):
    # df.to_numpy, list
    ...
    for c_list in [tables]:
        ...
        cur_column_count = []
        for c in c_list:
            p, q, r, s, cnt = get_embedded_table(c)
        ...
        table_embeds.append(TABGPT_ENCODER(anchor_table_input_ids, anchor_table_attention_mask, anchor_table_token_type_ids, anchor_table_mask, inference=True))
...
class TableEncoder(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(model_path, subfolder=subfolder)
        
        # initialize sentence transformer
        # self.st_name = SENTENCE_TRANSFORMER
        # model_dir = f"{MODELS_PATH}/{self.st_name}"
        self.st_name = st_name
        # TODO: load empty model first
        if self.st_name == 'all-MiniLM-L6-v2' or self.st_name == 'bge-small-en-v1.5':
            cfg = AutoConfig.from_pretrained(model_path,subfolder=subfolder)
            self.st = AutoModel.from_config(cfg)
            self.dim = self.st.config.hidden_size
        elif self.st_name == 'puff-base-v1':
            vector_dim = 768
            self.dim = vector_dim
            self.st = AutoModel.from_config(model_path,subfolder=subfolder)
