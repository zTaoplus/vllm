...
class ChatCompletionContentPartTabularParam(TypedDict, total=False):
    table: Required[Union[ColumnsTable, MarkdownTable]]
...
ModalityStr = Literal["image", "audio", "video", "table"]
_T = TypeVar("_T")
...
class BaseMultiModalItemTracker(ABC, Generic[_T]):
    ...
    def _placeholder_str(self, modality: ModalityStr,
        ...
        if modality == "image":
            ...
            if model_type.startswith("tablegpt"):
...
class MultiModalContentParser(BaseMultiModalContentParser):
    ...
    def parse_table(self, table) -> None:
        _table = get_and_parse_table(table)

        placeholder = self._tracker.add("table", _table)
        self._add_placeholder(placeholder)
...
class AsyncMultiModalContentParser(BaseMultiModalContentParser):
    ...
    def parse_table(self, table) -> None:
        _table = async_get_and_parse_table(table)
...
_TabularParser = partial(cast, ChatCompletionContentPartTabularParam)
...
MM_PARSER_MAP: Dict[str, Callable[[ChatCompletionContentPartParam], str]] = {
    ...
    "table":
    lambda part: _TabularParser(part).get("table", ""),
}
...
def _parse_chat_message_content_part(
    ...
    if part_type == "table":
        mm_parser.parse_table(content)
        return {'type': 'table'} if wrap_dicts else None
