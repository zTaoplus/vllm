...
from vllm.inputs import InputContext, token_inputs,DecoderOnlyInputs,DummyData
...
def get_encoder_output(
    ...
    # split
    splited_input_ids = torch.split(anchor_tables["input_ids"],
                                    _table_area_values)

    splited_token_type_ids = torch.split(anchor_tables["token_type_ids"],
                                         _table_area_values)
    splited_attention_mask = torch.split(anchor_tables["attention_mask"],
                                         _table_area_values)
...
def input_processor_for_tablegpt_encoder(ctx: InputContext,
    ...
    if prompt is None:
        prompt = decoder_tokenizer.decode(prompt_token_ids)
    
    # TODO: re tokenized for string prompt
    # 1. extract table infos
    table_infos = get_full_tables_info(mm_data["table"],
                                       ctx.model_config,
                                       return_text=False)
...
def get_table_max_cols_rows(
    hf_config: TableGPTContrastiveConfig, ) -> t.Tuple[int, int]:
...
def dummy_seq_data_for_contrastive_tablegpt(
    ...
    max_table_input_tokens = 3 * encoder_table_max_col * num_tables

    # -2 indicates the number of bos and eos tokens to be deleted
    if seq_len - max_table_input_tokens - 2 < 0:
        raise RuntimeError(
    ...
    return SequenceData.from_prompt_token_counts(
        (table_token_insert_id, max_table_input_tokens),
        (0, seq_len - max_table_input_tokens),
    ), {mm_key: consecutive_placeholder_ranges(num_items=num_tables,
                                    item_size= 3 * encoder_table_max_col)}
...
def dummy_data_for_contrastive_tablegpt(ctx: InputContext, seq_len: int,
    ...
    seq_data, ranges = dummy_seq_data_for_contrastive_tablegpt(
        hf_config, seq_len, num_tables)

    mm_data = dummy_tabledata_for_contrastive_tablegpt(ctx.model_config,
                                                       table_max_rows,
                                                       table_max_cols,
                                                       num_tables)
...
class Qformer(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None:
            ...
            mask = (mask[:,
                         0, :].unsqueeze(1).unsqueeze(1).unsqueeze(1).repeat(
                ...
                self.layers.append(
                    nn.ModuleList([
                        ...
                        PreNorm(
                            dim * nfeats,
                            Residual(
                                FeedForward(dim * nfeats, dropout=ff_dropout)),
                        ),
                    ]))
