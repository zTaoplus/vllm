...
class MultiHeadProjector(nn.Module):
    ...
    def __init__(self, config: PretrainedConfig):
        ...
        Args:
            config (object): PretrainedConfig: TableGPT Config
        ...
        if mlp_gelu_match:
            ...
            modules = [
                nn.Linear(self.encoder_hidden_size,
                          self.decoder_hidden_size * self.num_heads)
            ]
            for _ in range(1, mlp_depth):
                modules.append(nn.GELU())
                modules.append(
                    nn.Linear(self.decoder_hidden_size * self.num_heads,
                              self.decoder_hidden_size * self.num_heads))
    ...
    def prepare_insert_embeds(self,
                              *,
                              decoder,
                              input_ids,
                              position_ids=None,
        ...
        new_input_embeds = []
        num_insert_embs = int(
            (input_ids == self.config.encoder_config.insert_embs_token_id
             ).sum())
        ...
        insert_emb_token_indices = [-3] + indices.tolist() + [
            input_ids.shape[0]
        ]
        ...
        for i in range(len(insert_emb_token_indices) - 1):
            cur_input_ids_noim.append(input_ids[insert_emb_token_indices[i] +
                                                3:insert_emb_token_indices[i +
                                                                           1]])

        split_sizes = [x.shape[0] for x in cur_input_ids_noim]
        cur_input_embeds = decoder.get_input_embeddings(
        ...
        for i in range(n + 1):
            ...
            if i < n:
                cur_insert_emb_features = cur_table_embeds[
                    i]  # num_heads * decode_hidden
                if self.multihead:
                    assert cur_insert_emb_features.shape == (
                        self.num_heads, self.decoder_hidden_size
                    ), f"not match: {cur_insert_emb_features.shape}, \
                        {(self.num_heads), self.decoder_hidden_size}"
        ...
        for i, cur_new_embed in enumerate(new_input_embeds):
            ...
            if getattr(self.config.text_config, 'tokenizer_padding_side',
                       'right') == "left":
