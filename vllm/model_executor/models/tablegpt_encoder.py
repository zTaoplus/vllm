...
from transformers import PretrainedConfig,PreTrainedTokenizer,BertConfig
...
def get_embedded_table(table:Table, model_config:ModelConfig,tokenizer:PreTrainedTokenizer):
    ...
    anchor_table = tokenizer(anchor_table.tolist(), padding='max_length', truncation=True, max_length=model_config.hf_config.encoder_max_length, return_tensors='pt')                
    anchor_table = {k: v.reshape(anchor_row_num, num_cols, -1) for k, v in anchor_table.items()}

    num_cols = anchor_table['input_ids'].shape[1]

    anchor_table_row_num = anchor_table['input_ids'].shape[0]

    anchor_table_padded = {k: F.pad(v, (0, 0, 0, table_max_cols - v.shape[1], 0, table_max_rows - v.shape[0]), "constant", 1) for k, v in anchor_table.items()}
...
def get_encoder_output(tables:t.List[Table],model_config:ModelConfig,tokenizer:PreTrainedTokenizer):
    ...
    for table_list in [tables]:
        ...
        anchor_table_input_ids = torch.stack(anchor_table_input_ids, dim=0).to(device=TABGPT_ENCODER.st.device)
        anchor_table_attention_mask = torch.stack(anchor_table_attention_mask, dim=0).to(device=TABGPT_ENCODER.st.device)
        anchor_table_token_type_ids = torch.stack(anchor_table_token_type_ids, dim=0).to(device=TABGPT_ENCODER.st.device)
        anchor_table_mask = torch.stack(anchor_table_mask, dim=0).to(device=TABGPT_ENCODER.st.device)

        table_embeds.append(TABGPT_ENCODER(anchor_table_input_ids, anchor_table_attention_mask, anchor_table_token_type_ids, anchor_table_mask))
...
def input_processor_for_qwen2tb_encoder(ctx:InputContext, llm_inputs:LLMInputs):
    ...
    tokenizer = cached_get_tokenizer(ctx.model_config.model, subfolder=ctx.model_config.hf_config.encoder_config.subfolder)

    table_embeddings = get_encoder_output(mm_data["table"], model_config=ctx.model_config,tokenizer=tokenizer)
