# coding=utf-8
# Copyright 2023 Salesforce authors, The EleutherAI, and HuggingFace Teams.
# All rights reserved.
"""PyTorch CodeT5+ 2B 6B 16B models.
The implementation is mainly based on
transformers.models.codegen.modeling_codegen by adding cross-attention and
...
def fixed_pos_embedding(x, seq_dim=1, seq_len=None):
    ...
    sinusoid_inp = (torch.einsum("i , j -> i j",
                                 torch.arange(seq_len, dtype=torch.float),
...
def duplicate_interleave(m):
    """
    A simple version of `torch.repeat_interleave`
    for duplicating a matrix while interleaving the copy.
    """
    dim0 = m.shape[0]
...
def apply_rotary_pos_emb(x, sincos, offset=0):
    sin, cos = (duplicate_interleave(t)[None, offset:x.shape[1] + offset,
                                        None, :] for t in sincos)
    # einsum notation for
    # lambda t: repeat(t[offset:x.shape[1]+offset,:], "n d -> () n () (d j)", j=2)  # noqa: E501
...
class CodeT5pAttention(nn.Module):
    ...
    def __init__(self, config, is_cross_attention=False, is_decoder=True):
        ...
        if self.head_dim * self.num_attention_heads != self.embed_dim:
            raise ValueError(
                f"embed_dim must be divisible by num_attention_heads "
                f"(got `embed_dim`: {self.embed_dim} and"
                f" `num_attention_heads`: {self.num_attention_heads}).")

        self.scale_attn = torch.sqrt(
            torch.tensor(self.head_dim,
                         dtype=torch.float32)).to(torch.get_default_dtype())
    ...
    def _split_heads(self, x, n_head, dim_head, mp_num):
        ...
        reshaped = reshaped.reshape(x.shape[:-2] + (-1, ) +
                                    reshaped.shape[-1:])
        return reshaped
    ...
    def _merge_heads(self, tensor, num_attention_heads, attn_head_size):
        ...
        if len(tensor.shape) == 5:
            ...
            raise ValueError(f"Input tensor rank should be one of [4, 5], \
                    but is: {len(tensor.shape)}")
        new_shape = tensor.size()[:-2] + (num_attention_heads *
                                          attn_head_size, )
    ...
    def _attn(
        ...
        if not self.is_cross_attention and self.is_decoder:
            ...
            causal_mask = self.causal_mask[:, :, key_length - query_length:
    ...
    ) -> Union[Tuple[torch.Tensor, Tuple[torch.Tensor]], Optional[Tuple[
            torch.Tensor, Tuple[torch.Tensor], Tuple[torch.Tensor, ...]]], ]:
        if encoder_hidden_states is not None:
            if not hasattr(self, "q_attn"):
                raise ValueError(
                    "If class is used as cross attention, the weights `q_attn` have to be defined. "  # noqa: E501
                    "Please make sure to instantiate class with `GPT2Attention(..., is_cross_attention=True)`."  # noqa: E501
                )
...
class CodeT5pMLP(nn.Module):
    ...
    def __init__(self, intermediate_size,
                 config):  # in MLP: intermediate_size= 4 * embed_dim
        super().__init__()
    ...
    def forward(
            self,
            hidden_states: Optional[torch.FloatTensor]) -> torch.FloatTensor:
...
class CodeT5pBlock(nn.Module):
    ...
    def __init__(self, config, layer_idx=None):
        ...
        self.add_cross_attention_by_layer = bool(
            config.add_cross_attention and layer_idx == config.n_layer - 1)

        if config.add_cross_attention and self.add_cross_attention_by_layer:
            self.crossattention = CodeT5pAttention(config,
    ...
    def forward(
        ...
        if (encoder_hidden_states is not None
                and self.add_cross_attention_by_layer):
            # add one self-attention block for cross-attention
            if not hasattr(self, "crossattention"):
                raise ValueError(
                    f"If `encoder_hidden_states` are passed, {self} has to be \
                        instantiated with cross-attention layers by \
...
# Adapted from transformers.models.codegen.modeling_codegen.CodeGenPreTrainedModel  # noqa: E501
...
class CodeT5pModel(CodeT5pPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.h = nn.ModuleList(
            [CodeT5pBlock(config, idx) for idx in range(config.n_layer)])
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            # this attention mask is more simple than
            # the triangular masking of causal attention
            # used in OpenAI GPT,
        ...
        for i, (block, layer_past) in enumerate(zip(self.h, past_key_values)):
            ...
            if self.gradient_checkpointing and self.training:
                if use_cache:
                    logger.warning("`use_cache=True` is incompatible with \
                            `config.gradient_checkpointing=True`. Setting "
                                   "`use_cache=False`...")
                    use_cache = False
